...
class AddExpenseViewModel(private val transactionRepository: TransactionRepository) : ViewModel() {
    ...
    fun saveExpense(amountStr: String, description: String?, category: String, date: String, account: String = "Efectivo") {
        ...
        viewModelScope.launch {
            try {
                ...
                Log.d("AddExpenseVM", "Guardando gasto con userId: $actualUserId, categoría: $category, cuenta: $account, detalles: $newExpense")
    ...
    fun updateExpense(transactionId: String, amountStr: String, description: String?, category: String, date: String, account: String = "Efectivo") {
        if (amountStr.isBlank()) {
            _validationError.value = "El monto no puede estar vacío"
            return
        }
        ...
        viewModelScope.launch {
            try {
                val currentSupabaseUser = supabaseClient.auth.currentUserOrNull()
                if (currentSupabaseUser == null) {
                    Log.e("AddExpenseVM", "Usuario no autenticado. No se puede actualizar el gasto.")
                    _saveOperationResult.value = Result.failure(Exception("Usuario no autenticado."))
                    _isLoading.value = false
                    return@launch
                }
                ...
                Log.d("AddExpenseVM", "Actualizando gasto con ID: $transactionId")
                val result = transactionRepository.updateExpense(updatedExpense)
                _saveOperationResult.value = result

            } catch (e: Exception) {
                Log.e("AddExpenseVM", "Excepción al actualizar gasto: ${e.message}", e)
                _saveOperationResult.value = Result.failure(e)
            } finally {
                _isLoading.value = false
            }
